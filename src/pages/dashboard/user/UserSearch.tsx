      ...
      <div className="p-6 max-w-6xl">
        ...
        <div className="flex gap-3 mb-6">
          <div className="flex-1 flex items-center gap-2 bg-white rounded-2xl border border-slate-200 px-4 shadow-sm">
            ...
            <input value={q} onChange={e => { setQ(e.target.value); setPage(1); }} placeholder="Search restaurants, gyms, cafes..." className="flex-1 py-3 text-sm bg-transparent text-slate-800 placeholder-slate-400 focus:outline-none" />
          </div>
          <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="input-field py-3 text-sm w-auto px-4">
        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-2 mb-6">
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => { setCategory(cat); setPage(1); }} className={`whitespace-nowrap px-4 py-2 rounded-xl text-sm font-medium transition-all ${category === cat ? 'bg-primary-500 text-white shadow-md' : 'bg-white text-slate-600 border border-slate-200 hover:border-primary-300'}`}>
              {cat}
            </button>
          ))}
        </div>

        <p className="text-slate-500 text-sm mb-5"><span className="font-bold text-slate-900">{filtered.length}</span> businesses found</p>
