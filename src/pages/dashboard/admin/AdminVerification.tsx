  ...
  return (
    <div>
      ...
      <div className="p-6 max-w-5xl">
        <div className="flex items-center gap-3 mb-6 bg-amber-50 border border-amber-200 rounded-2xl p-4">
          <ShieldCheck className="w-5 h-5 text-amber-500 flex-shrink-0" />
          <p className="text-sm text-amber-700 font-medium">{pending.length} listings awaiting verification review</p>
        </div>
                    ...
                    <div className="flex items-start justify-between gap-4 flex-wrap">
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className="badge bg-slate-100 text-slate-600"><Building2 className="w-3 h-3" /> {p.category}</span>
                          <span className="badge bg-amber-100 text-amber-700"><Clock className="w-3 h-3" /> Pending</span>
                        </div>
                        <div className="text-xs text-slate-400 mt-2 space-y-0.5">
                          <p>Owner: <span className="text-slate-600 font-medium">{p.owner}</span></p>
                          <p>Submitted: <span className="text-slate-600">{p.submitted}</span></p>
                          <p>Documents: <span className="text-slate-600">{p.docs} files attached</span></p>
                        </div>
                      </div>
                      <div className="flex gap-2 flex-shrink-0">
                        <button onClick={() => toast.success('Opening document preview...')} className="flex items-center gap-1.5 px-3 py-2 border border-slate-200 rounded-xl text-sm text-slate-600 hover:bg-slate-50 transition-colors">
